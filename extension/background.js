...
// src/background/dictionary-service.js
...
async function checkWords(words, personal = []) {
...
var WORDS_PREFIX = "words:";
var WORDS_PER_CHUNK = 200;
...
chrome.runtime.onMessage.addListener((message, _sender, sendResponse) => {
  ...
  handler(message).then(sendResponse).catch((error) => sendResponse({ error: String(error?.message || error) }));
  return true;
});
